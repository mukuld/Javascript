...
export const handler = async (event) => {
  ...
  const apiUrl = "https://www.servicenow.com/community/architect-forum/insertmultiple-import-set-api/m-p/2442022";

  const postData = objectData;

  async function postToSNOW(apiUrl, postData) {
    try {
  ...
  try {
    ...
    for (const objectKey of objectKeys) {
      ...
      const postSNOW = await postToSNOW(apiUrl, postData);
